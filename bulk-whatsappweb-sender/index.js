...
global.pupPath = ""; // '' means that the path is the deault one
...
__dirname.startsWith(path.join(os.tmpdir(), "caxa"))
...
program
  ...
  .name("npm run start")
  .description(
    "Send bulk messages to a telephone number list. \nStart and then follow the instructions."
  )
...
program
  ...
  .option(
    "-m, --message [msg]",
    "write message directly as a parameter, overwrites '--text-file'"
  )
  .option(
    "-M, --no-message",
    "do not send text, overwrites '--message' and '--text-file'"
  )
  ...
  .option(
    "-d, --low-delay",
    "send messages with a low delay, use this if you are confident that you won't be banned"
  )
  .option(
    "-D, --high-delay",
    "send messages with a high delay, use this if you are sending from a new/unused number (high probability of being banned)"
  )
  .option(
    "-la, --local-auth",
    "use LocalAuth authentication mode instead of NoAuth (keep your account logged in)",
    false
  )
  .option(
    "-lc, --local-chromium",
    "use local Chromium executable instead of installed Chrome",
    false
  );
...
if (compiled) pressAnyKey("Press any key to exit..."); //TODO DOES NOT WAIT!
...
async function downloadLocalChromium() {
  ...
  const revisionInfo = await browserFetcher.download(
    acceptedRevs.PUPPETEER_REVISIONS.chromium
  );
