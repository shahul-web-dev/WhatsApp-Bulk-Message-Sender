...
def delete_rows_except_first(file_path):
    # Load the workbook
    ...
    for row in reversed(range(2, max_row + 1)):
...
# Call the function
delete_rows_except_first(file_path)

# Provide the path to your Excel file
file_path = "./NotSent.xlsx"
...
# Load the Excel file
workbook = openpyxl.load_workbook('./num.xlsx')

# Select the first sheet
sheet = workbook.active

# Read data from Excel into a list
data = []
...
def find_BBWBS_cli():
    global BBWWS_NODE_FOLDER
    bwwbs_node_path_not_compiled = os.path.abspath(os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "..", "bulk-whatsappweb-sender"))
    if getattr(sys, 'frozen', False):   # if compiled
        BBWWS_NODE_FOLDER = os.path.abspath(os.path.join(
            os.path.dirname(sys.executable), "bulk-whatsappweb-sender"))
    elif os.path.exists(bwwbs_node_path_not_compiled):
        BBWWS_NODE_FOLDER = bwwbs_node_path_not_compiled
        return
    else:
        tkmessagebox.showerror(
            "Error", "Bulk WhatsAppWeb Sender is not present!")
        exit()


def init_BBWBS_cli():
    try:
        subprocess.run("node -v", stdout=subprocess.DEVNULL,
...
def get_BWWBS_cli_version():
    global BBWWS_NODE_FOLDER
    global VERSION
    out = subprocess.run("npm view . version",
                         cwd=BBWWS_NODE_FOLDER, shell=True, capture_output=True)
...
def popup_update(tk_root):  # TODO window does not appear on top
    ...
    label_pop = ttk.Label(
        pop, text="A new version of Bulk-WhatsAppWeb-Sender is available!")
    ...
    link = ttk.Label(pop, text="https://github.com/albertopasqualetto/Bulk-WhatsappWeb-Sender/releases/latest",
                     foreground="blue", cursor="hand2")
    link.grid(row=1, column=1, padx=1)
    link.bind("<Button-1>", lambda e: webbrowser.open_new(
        "https://github.com/albertopasqualetto/Bulk-WhatsappWeb-Sender/releases/latest"))
...
def start_BBWBS_cli():
    ...
    if NUMBERS_FILE == "":
        selected_numbers_file_path_label.config(
            text="Please select a file!", foreground="red")
        return
...
if __name__ == '__main__':
    ...
    numbers_file_button = ttk.Button(
        root, text='📞 Select Numbers File', command=select_numbers_file)
    ...
    selected_numbers_file_path_label.grid(
        row=0, column=1, columnspan=2, sticky='E', padx=10, pady=10)

    message_label = ttk.Label(root, text='✉️ Message:')
    message_label.grid(row=1, column=0, sticky='W', padx=10, pady=10)
    # TODO handle emojis properly
    ...
    media_to_send_button = ttk.Button(
        root, text='📷 Select Media to Send', command=select_media_files)
    media_to_send_button.grid(row=2, column=0, sticky='W', padx=10, pady=10)
    selected_media_to_send_paths_label = ttk.Label(
        root, text=MEDIA_TO_SEND_LIST)
    selected_media_to_send_paths_label.grid(
        row=2, column=1, sticky='E', padx=10, pady=10)
    clear_media_to_send_button = ttk.Button(
        root, text='❌', command=clear_selected_media_files, width=3)
    ToolTip(clear_media_to_send_button, 'Clear all selected media', delay=1.0)
    clear_media_to_send_button.grid(
        row=2, column=2, sticky='E', padx=10, pady=10)

    delay_label = ttk.Label(root, text='⏱️ Delay:')
    delay_label.grid(row=3, column=0, sticky='W', padx=10, pady=10)
    # 0 means default delay, -1 means low delay, 1 means high delay
    delay_tk_var = tk.StringVar(None, '0')
    radio_low = ttk.Radiobutton(
        root, text='Low Delay', variable=delay_tk_var, value=-1)
    radio_default = ttk.Radiobutton(
        root, text='Default Delay', variable=delay_tk_var, value=0)
    radio_high = ttk.Radiobutton(
        root, text='High Delay', variable=delay_tk_var, value=1)
    ...
    radio_default.grid(row=3, column=1, columnspan=2,
                       sticky='N', padx=10, pady=10)
    radio_high.grid(row=3, column=1, columnspan=2,
                    sticky='E', padx=10, pady=10)
    ...
    send_button = ttk.Button(
        root, text='🚀 SEND MESSAGES', command=start_BBWBS_cli)
    send_button.grid(row=5, column=0, columnspan=3,
                     sticky='EW', padx=10, pady=5)

    continue_in_terminal_label = ttk.Label(
        root, text='Then continue in terminal and scan the QR code...')
    continue_in_terminal_label.grid(
        row=6, column=0, columnspan=3, sticky='N', padx=10, pady=5)
